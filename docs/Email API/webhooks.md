...
​Webhook is a mechanism for asynchronously receiving event notifications. After you submit an email delivery request to Aurora SendCloud, the system synchronously returns the request result (e.g., whether the request was accepted). The final delivery result of the email (e.g., whether it was successfully delivered) and other interaction events (e.g., opens, clicks) are asynchronously pushed to your specified server via Webhook.

Instead of requiring you to poll for updates, Aurora SendCloud actively pushes data to you in real-time when key events occur. This enables subsequent processes like data synchronization, statistical analysis, or triggering business workflows.
...
​​Event Occurs:​​ When an email-related event occurs (e.g., an email is opened, a recipient clicks a link, the email is bounced), Aurora SendCloud generates corresponding data.
...
​​Receive & Process:​​ Your server receives the request, parses the data, and handles subsequent business logic (e.g., updating a database, sending an internal alert) based on the event type.
...
Write code to parse the JSON data in the POST request body and handle it accordingly based on the event type (e.g., identified by an eventfield).


​​Return Success Status:​​ 

After successfully receiving and processing the data, your server should return a 2xxHTTP status code (e.g., 200 OK) to Aurora SendCloud. If a non-2xx status code is returned, Aurora SendCloud may attempt to retry the notification based on its retry policy.
...
​​Processing Performance:​​ Ensure your receiving endpoint can respond quickly (e.g., within 50ms) to avoid timeouts that might cause Aurora SendCloud to misinterpret the delivery as a failure.
